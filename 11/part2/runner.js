...
const doMonkeyBusiness = (monkeys, rounds) => {
    ...
    const magicModulo = monkeys.reduce((acc, monkey) => acc * monkey.test, 1);
    ...
    for (let round = 1; round <= rounds; round++) {
        ...
        for (const monkey of monkeys) {
            while (monkey.items.length > 0) {
                ...
                worryLevel = monkey.operation(worryLevel) % magicModulo;
                let dest;
...
console.log(doMonkeyBusiness(monkeys, 10000));